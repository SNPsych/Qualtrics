...
## This script requires all following packages, plus "pandoc" and "pdflatex"
...
fig.timing <- ggplot(d[User == ID], aes(Date)) +
...
fig.sol <- ggplot(d[User == ID], aes(Date)) +
  ...
  scale_y_continuous(breaks = sort(c(seq(min(d[User == ID, SOL], na.rm = TRUE), max(d[User == ID, SOL], na.rm = TRUE), by = 10), 30))) +
  labs(x = "", y = "Minutes") +
  scale_x_date(labels = date_format("%d/%m"), breaks = date_breaks("1 day")) +
  theme(axis.text.x = element_text(angle = 90, size = 12),
        axis.text.y = element_text(size = 12),
...
fig.waso <- ggplot(d[User == ID], aes(Date)) +
  geom_line(aes(y = WASOT), colour = "#5e3c99", size =1.5) +
  geom_point(aes(y = WASOT), colour = "#5e3c99", size =3.5) +
  geom_hline(yintercept=30, size = 0.5, linetype = "dashed", colour = "#e66101") +
  scale_y_continuous(breaks = sort(c(seq(min(d[User == ID, WASOT], na.rm = TRUE), max(d[User == ID, WASOT], na.rm = TRUE), by = 10), 30))) +
  labs(x = "", y = "Minutes") +
  scale_x_date(labels = date_format("%d/%m"), breaks = date_breaks("1 day")) +
  theme(axis.text.x = element_text(angle = 90, size = 12),
        axis.text.y = element_text(size = 12),
...
fig.SE <- ggplot(d[User == ID], aes(Date)) +
...
fig.duration <- ggplot(d[User == ID], aes(Date)) +
...
- Stayed in bed for **`r round(mean(d[User == ID][(Date > (Date[.N] - 6))]$TIBhr, na.rm = TRUE), digits = 1)`** hours.
- Spent **`r round(mean(d[User == ID][(Date > (Date[.N] - 6))]$TSTshr, na.rm = TRUE), digits = 1)`** hours' sleep per night.
- Took **`r round(mean(d[User == ID][(Date > (Date[.N] - 6))]$SOL, na.rm = TRUE), digits = 1)`** minutes to fall asleep.
- Spent **`r round(mean(d[User == ID][(Date > (Date[.N] - 6))]$WASOT, na.rm = TRUE), digit = 1)` minutes** awake at night.
- Had a sleep efficiency of **`r round(mean(d[User == ID][(Date > (Date[.N] - 6))]$SEs, na.rm = TRUE), digit = 1)`%**.
